...
test('clicking a restaurant card centers the map', async ({ page }) => {
  ...
  const map = page.locator(mapContainerSelector);
  const getMapCenter = async () => {
    return await page.evaluate(() => {
      // @ts-ignore // Accessing Leaflet's map object
      const mapInstance = (document.querySelector('.leaflet-container') as any)?._leaflet_map;
      if (mapInstance) {
